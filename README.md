...
3. 在**工具分配**中选择对应的工具组
4. 在 **Description for main LLM** 中填写以下描述（主智能体据此决定何时委托）：

| 子智能体 | Description for main LLM |
|----------|--------------------------|
| `market_analyst` | 负责股票技术面分析，可调用K线、实时行情和资金流向数据。用于分析价格趋势、均线系统、MACD/KDJ/RSI等技术指标、支撑压力位，并给出技术面投资建议。当需要技术面分析时调用此子智能体。 |
| `fundamentals_analyst` | 负责股票基本面分析，可调用资产负债表、利润表、现金流量表。用于分析盈利能力（ROE/毛利率/净利率）、估值水平（PE/PB）、成长性（营收/利润增长率）、财务健康（资产负债率/现金流），并给出基本面投资建议。当需要基本面分析时调用此子智能体。 |
| `news_analyst` | 负责宏观经济与市场情绪分析，可调用宏观经济指标（GDP/CPI/PMI/LPR/汇率/商品期货）和资金流向数据。用于分析宏观环境、行业政策、市场情绪、利好利空因素。当需要宏观或情绪面分析时调用此子智能体。 |
| `bull_researcher` | 负责深度挖掘股票的利好因素，可调用K线、行情和资金流向数据。从估值修复、业绩增长、政策利好、市场情绪、技术面支撑、竞争优势等角度寻找支撑股价上涨的证据。收到三份分析报告后给出多方位利好论点。 |
| `bear_researcher` | 负责深度挖掘股票的利空因素，可调用K线、行情和资金流向数据。从估值泡沫、业绩下滑、政策风险、市场竞争、技术面压力、资金流出、宏观经济风险等角度寻找可能导致股价下跌的风险。收到三份分析报告后给出多方位利空论点。 |
| `research_manager` | 负责综合多方和空方研究报告，生成客观的辩论综合报告。不调用任何工具，仅基于传入的多空双方报告进行对比分析，指出共识和分歧，给出综合投资建议。当多空双方都完成研究后调用此子智能体。 |
| `risk_judge` | 负责最终风险评估，可调用K线、行情、资金流向和宏观经济数据。综合所有分析报告给出明确风险等级（极高/高/中/低/极低），分解不同级别的风险因素，评估风险收益比，提供风险缓解建议。在所有分析完成后最后调用此子智能体。 |
